...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Removed dead helpers (`getFindingDescription`, `getFindingName`, generateUnifiedAuditFindingsHTML,
    // generateGitHubActionsAuditHTML). Their KPI cards (Total/High/Medium/Finding Types) were never
    // rendered because `generateAllAuditSectionsHTML` did not call them; canonical implementations
    // live in `js/common.js` and the findings UI lives in `findings.html` / `js/findings-page.js`.
    // Their KPI cards (Total/High/Medium/Finding Types) were never rendered because
    // generateAllAuditSectionsHTML did not call them; findings UI lives in findings.html.
    ...
    // Note: Version Drift, Stale Dependencies, EOX, and License Compatibility sections
    // have been removed from the audit page. These features are available on the Deps page.
    ...
    async function generateSBOMAuditHTML(orgData, severityFilter = 'all', repoFilter = 'all') {
        ...
        filteredRepos.forEach(repo => {
            ...
            reposWithSBOM++;
            // Guard against unexpected grade values (anything we didn't
            // pre-populate). Without this, `gradeDistribution[undefined]++`
            // produces NaN totals downstream — same risk as the aggregator
            // path fixed in batch 1.
            const gradeKey = (quality.grade && Object.prototype.hasOwnProperty.call(gradeDistribution, quality.grade))
                ? quality.grade
                : 'N/A';
            ...
            // Calculate a "completeness-style" score using the categories
            // that survive the sbomqs v2.0 schema upgrade. The old code
            // referenced `metadata.score`, which no longer exists on new
            // assessments and was always 0, dragging the average down.
            // Each category score is already on a 0-100 scale, so the
            // weighted sum is also 0-100 — fine to display as %.
            const compCat = quality.categories?.completeness?.score
                ?? ((quality.categories?.dependencies?.score || 0) + (quality.categories?.metadata?.score || 0)) / 2
                ?? 0;
        ...
        html += `<div class="col-md-3">
            <div class="card text-center bg-secondary bg-opacity-10" title="Weighted average of identification (30%), completeness (30%), licensing (20%), and vulnerability (20%) category scores. Each category is already a 0-100 score, so this is points out of 100 — not a percentage of repositories.">
                <div class="card-body">
                    <h3>${reposWithSBOM > 0 ? Math.round(totalCompleteness / reposWithSBOM) : 0}<small class="text-muted">/100</small></h3>
                    <p class="stats-card-label mb-0">Avg Completeness <small class="text-muted d-block">weighted points</small></p>
                </div>
            </div>
        </div>`;
        html += '</div>';

/**
 * JavaScript for authors.html page
 * Main page initialization and author display logic
...
(async function initAuthorsPage() {
        document.addEventListener('DOMContentLoaded', async function() {
            console.log('👥 Authors Page - Initializing...');
            ...
            function getEcosystemIcon(ecosystem) {
                ...
                return iconMap[normalized] || 'fas fa-cube';
            }
            
            // Get ecosystem badge color
            function getEcosystemBadgeClass(ecosystem) {
                const colorMap = {
                    'npm': 'bg-danger',
                    'pypi': 'bg-primary',
                    'maven': 'bg-warning text-dark',
                    'go': 'bg-info text-dark',
                    'golang': 'bg-info text-dark',
                    'cargo': 'bg-secondary',
                    'nuget': 'bg-purple',
                    'packagist': 'bg-indigo',
                    'composer': 'bg-indigo',
                    ...
                    'gem': 'bg-danger',
                    'docker': 'bg-primary',
                    'githubactions': 'bg-dark',
                    'github actions': 'bg-dark',
                    'terraform': 'bg-purple',
                    'helm': 'bg-info'
                };
                const normalized = (ecosystem || '').toLowerCase();
                return colorMap[normalized] || 'bg-secondary';
            }
            
            // Generate ecosystem badge HTML with icon
            function getEcosystemBadge(ecosystem) {
                const icon = getEcosystemIcon(ecosystem);
                const badgeClass = getEcosystemBadgeClass(ecosystem);
                const displayName = ecosystem || 'Unknown';
                return `<span class="badge ${badgeClass}"><i class="${icon} me-1"></i>${escapeHtml(displayName)}</span>`;
            }
                        ...
                        if (window.cacheManager && author.authorKey) {
                            try {
                                ...
                                if (authorEntity) {
                                    ...
                                    if (countryCodesArray.length === 1) {
                                        console.log(`💾 Saved country code ${countryCodesArray[0]} for author ${author.authorKey} to IndexedDB`);
                                    } else {
                                        console.log(`💾 Saved ${countryCodesArray.length} country codes (${countryCodesArray.join(', ')}) for author ${author.authorKey} to IndexedDB`);
                                    }
                                } else {
                                    console.warn(`⚠️ Could not find author entity ${author.authorKey} to save country code`);
                                }
                            } catch (error) {
                                console.error(`❌ Failed to save country code for ${author.authorKey}:`, error.message);
                            }
                        } else {
                            console.warn(`⚠️ Cannot save country code: cacheManager=${!!window.cacheManager}, authorKey=${!!author.authorKey}`);
                        }
                    }
                } catch (error) {
            ...
            console.log('✅ Authors Page - Storage initialized');
            
            // Load available analyses
            await loadAnalysisList();
            console.log('✅ Authors Page - Analysis list loaded');
            ...
            async function displayAuthorMap(authors) {
                ...
                if (!mapContainer || mapContainer.offsetParent === null) {
                    console.warn('⚠️ Map container is not visible, skipping marker display');
                    return;
                }
            ...
            // Load initial data
            console.log('📊 Authors Page - Loading initial author data...');
            await loadAuthorData();
            
            async function loadAnalysisList() {
                try {
                    console.log('📋 Loading analyses list for authors page...');
                    ...
                    console.log('📋 Authors Page - Storage info:', storageInfo);
                    console.log(`📋 Authors Page - Organizations: ${storageInfo.organizations.length}, Repositories: ${storageInfo.repositories.length}`);
                    ...
                    console.log(`📋 Authors Page - Adding ${allEntries.length} entries to selector`);
                    ...
                    allEntries.forEach(entry => {
                        ...
                        console.log(`📋 Added option: ${entry.name}`);
                    });
                    
                    selector.disabled = false;
                    console.log(`✅ Analysis selector populated with ${allEntries.length + 1} entries (including combined)`);
                } catch (error) {
            ...
            async function loadCombinedAuthors() {
                ...
                const allEntries = [...storageInfo.organizations, ...storageInfo.repositories];
                console.log(`📊 Authors Page - Combining authors from ${allEntries.length} entries`);
                
                for (const entry of allEntries) {
                    console.log(`📊 Authors Page - Loading data for: ${entry.name}`);
                    const data = await storageManager.loadAnalysisDataForOrganization(entry.name);
                    
                    if (data) {
                        console.log(`📊 Authors Page - Data structure for ${entry.name}:`, {
                            hasData: !!data.data,
                        ...
                        console.log(`⚠️ Authors Page - No data found for ${entry.name}`);
                    }
                    ...
                    if (!authorRefs && window.indexedDBManager && data?.data?.allDependencies) {
                        ...
                        console.log(`⚠️ Authors Page - authorAnalysis missing, rebuilding from cache for ${entry.name}`);
                        authorRefs = await rebuildAuthorAnalysisFromCache(data.data, entry.name);
                    }
                    
                    if (authorRefs && authorRefs.length > 0) {
                        console.log(`✅ Authors Page - Found ${authorRefs.length} authors in ${entry.name}`);
                ...
                console.log(`📊 Authors Page - Combined total: ${result.length} unique authors`);
                return result;
            }
            ...
            async function rebuildAuthorAnalysisFromCache(analysisData, identifier) {
                if (!window.indexedDBManager || !window.cacheManager) {
                    console.warn('⚠️ Cannot rebuild: indexedDBManager or cacheManager not available');
                    return [];
                }
                ...
                try {
                    ...
                    console.log(`✅ Rebuilt ${authorRefs.length} author references from cache`);
                    return authorRefs;
                } catch (error) {
                    console.error('❌ Failed to rebuild authorAnalysis from cache:', error);
                    return [];
                }
            }
            ...
            async function loadSingleAnalysisAuthors(analysisId) {
                ...
                if (data) {
                    console.log(`📊 Authors Page - Data for ${analysisId}:`, {
                        hasData: !!data.data,
                        hasAuthorAnalysis: !!data.data?.authorAnalysis,
                        hasAuthors: !!data.data?.authorAnalysis?.authors,
                        authorCount: data.data?.authorAnalysis?.authors?.length || 0,
                    ...
                    console.log(`⚠️ Authors Page - No data found for ${analysisId}`);
                }
                ...
                if (!authorRefs && window.indexedDBManager && data?.data?.allDependencies) {
                    // Rebuild authorAnalysis from cache if missing
                    console.log(`⚠️ Authors Page - authorAnalysis missing, rebuilding from cache for ${analysisId}`);
                ...
                if (isNewFormat && window.cacheManager) {
                    ...
                    console.log(`📦 Authors Page - Resolving ${authorRefs.length} author references from cache...`);
                    ...
                    console.log(`✅ Authors Page - Resolved ${authors.length} authors from cache`);
                    return authors;
                } else {
                    // OLD FORMAT: Return as-is (backward compatibility)
                    console.log(`📊 Authors Page - Using legacy format (${authorRefs.length} authors)`);
                    return authorRefs;
                }
            }
        ...
        async function validateUrl(url) {
            ...
            try {
                ...
                if (isUrlFromHostname(url, 'pypi.org')) {
        ...
        async function validateAndOpenUrl(url) {
            ...
            if (urlValidationCache.has(url)) {
                ...
                if (isValid) {
                    ...
                    alert(`⚠️ This URL appears to be invalid (404): ${url}`);
                }
                return;
            }
            ...
            try {
                ...
                if (isValid) {
                    ...
                    console.warn(`⚠️ URL validation failed for: ${url}`);
                    const userConfirm = confirm(`⚠️ Warning: This URL may not exist (404).\n\n${url}\n\nDo you want to open it anyway?`);
                    if (userConfirm) {
                        window.open(url, '_blank', 'noreferrer,noopener');
                    }
                }
                ...
                console.warn(`⚠️ URL validation error for ${url}:`, error);
                window.open(url, '_blank', 'noreferrer,noopener');
            }
        }
        ...
        function getSocialProfileIcons(author) {
                    ...
                    <div class="mb-3">
                        ...
                        <table class="table table-sm table-borderless">
                            ...
                            <tr>
                                ...
                                <td>${getEcosystemBadge(author.ecosystem)}</td>
                            </tr>
                            <tr>
                    ...
                    if (author.metadata.location) {
                        ...
                        if (countryCodesToCheck.length > 0 && sanctionsService) {
                            ...
                            if (sanctionedCountries.length > 0) {
                                ...
                                locationDisplay += ` <span class="badge bg-danger ms-2" title="Sanctioned country (${allSources}): ${escapeHtml(codes)}">🚫 ${allSources}</span>`;
                            }
                        }
                        html += `
                            <tr>
            ...
            function displayAuthors(authors, ecosystemFilter, locationFilter) {
                ...
                console.log(`🎨 Authors Page - Displaying authors. Total: ${authors ? authors.length : 0}, Filter: ${ecosystemFilter}, Location: ${locationFilter}`);
                
                if (!authors || authors.length === 0) {
                    console.log('⚠️ Authors Page - No authors to display');
                ...
                if (ecosystemFilter !== 'all') {
                    ...
                    console.log(`🎨 Authors Page - After filtering by ${ecosystemFilter}: ${filtered.length} authors`);
                }
                ...
                if (locationFilter !== 'all') {
                    ...
                    console.log(`🎨 Authors Page - After filtering by location ${locationFilter}: ${filtered.length} authors`);
                }
                ...
                if (fundingFilterCheckbox && fundingFilterCheckbox.checked) {
                    ...
                    console.log(`🎨 Authors Page - After filtering by funding: ${filtered.length} authors`);
                }
                ...
                if (sanctionsFilterActive && sanctionsService) {
                    ...
                    console.log(`🎨 Authors Page - After filtering by sanctioned countries: ${filtered.length} authors`);
                }
                ...
                if (filtered.length === 0) {
                    console.log('⚠️ Authors Page - No authors after filtering');
                    const fundingFilterCheckbox = document.getElementById('fundingFilter');
                ...
                if (sanctionsFilterActive) {
                    console.log(`🎨 Authors Page - Sanctions filter active: Showing ${visibleAuthors.length} of ${allVisibleHumanAuthors.length} human authors from sanctioned countries, ${organizationAuthors.length} organizations, ${botAuthors.length} bots`);
                } else {
                    ...
                    console.log(`🎨 Authors Page - Human Authors: ${allVisibleHumanAuthors.length} (${multiPackageCount} multi-package + ${singlePackageMultiRepoCount} single-package-multi-repo), Organizations: ${organizationAuthors.length}, Bots: ${botAuthors.length}, Hidden: ${singlePackageSingleRepo.length} (single-package-single-repo), Displaying: ${visibleAuthors.length} (limit: ${displayLimit})`);
                }
                ...
                let html = `
                    <div class="alert ${sanctionsFilterActive ? 'alert-danger' : 'alert-info'}">
                        ...
                        ${sanctionsFilterActive ? '<strong>⚠️ Sanctioned Countries Filter Active:</strong> ' : ''}
                ...
                visibleAuthors.forEach((author, index) => {
                    ...
                    if (author.metadata?.tentative_correlation) {
                        locationCell += ` <span class="badge bg-warning text-dark ms-1" title="Tentative correlation - same GitHub user ID">⚠️</span>`;
                    }
                    // Check for sanctioned countries (check all country codes if multiple)
                    const countryCodesToCheck = author.metadata?.countryCodes || (author.metadata?.countryCode ? [author.metadata.countryCode] : []);
                    if (countryCodesToCheck.length > 0 && sanctionsService) {
                        ...
                        if (sanctionedCountries.length > 0) {
                            const allSources = [...new Set(sanctionedCountries.flatMap(s => s.sources.split(', ')))].join(', ');
                            const codes = sanctionedCountries.map(s => s.code).join(', ');
                            locationCell += ` <span class="badge bg-danger ms-1" title="Sanctioned country (${allSources}): ${escapeHtml(codes)}">🚫 ${allSources}</span>`;
                        }
                    }
                    locationCell += '</td>';
                    ...
                    let socialCell = '<td class="text-center">—</td>';

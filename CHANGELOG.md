...
### Added
- **Hidden Quick demo page** ([`demo.html`](demo.html), [`js/demo-page.js`](js/demo-page.js)): open-only URL (not in the nav) with buttons that fetch bundled SBOM JSON from [`data/demo/`](data/demo/) and run the same pipeline as **Upload SBOM** (`processUploadedSBOM` → enrichment → IndexedDB).
- **Bundled demo SBOM** ([`data/demo/sbomplay-demo.json`](data/demo/sbomplay-demo.json)) for presenter-style demos on GitHub Pages (add more scenarios via `DEMO_SCENARIOS` in `js/demo-page.js`).
- **Experimental-accuracy banner on the Insights page** (`insights.html`): a prominent, non-dismissible warning at the top of the page informs users that the dashboard is under active development and data shown may be inaccurate or incomplete. The banner will be removed once the feature stabilises.
